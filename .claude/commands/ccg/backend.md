...
# 新会话调用
Bash({
  command: "C:/Users/saite/.claude/bin/codeagent-wrapper.exe --backend codex - \"{{WORKDIR}}\" <<'EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
...
# 复用会话调用
Bash({
  command: "C:/Users/saite/.claude/bin/codeagent-wrapper.exe --backend codex resume <SESSION_ID> - \"{{WORKDIR}}\" <<'EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
需求：<增强后的需求（如未增强则用 $ARGUMENTS）>
上下文：<前序阶段收集的项目上下文、分析结果等>
...
| 阶段 | Codex |
|------|-------|
| 分析 | `C:/Users/saite/.claude/.ccg/prompts/codex/analyzer.md` |
| 规划 | `C:/Users/saite/.claude/.ccg/prompts/codex/architect.md` |
| 审查 | `C:/Users/saite/.claude/.ccg/prompts/codex/reviewer.md` |
...
1. **代码检索**（如 ace-tool MCP 可用）：调用 `mcp__fast-context__fast_context_search` 检索现有 API、数据模型、服务架构
