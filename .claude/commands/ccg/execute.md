...
根据计划中的"关键文件"列表，调用 `mcp__fast-context__fast_context_search` 检索相关代码：
...
1. 调用 Gemini（使用 `C:/Users/saite/.claude/.ccg/prompts/gemini/frontend.md`）
...
1. 调用 Codex（使用 `C:/Users/saite/.claude/.ccg/prompts/codex/architect.md`）
2. 输入：计划内容 + 检索到的上下文 + 目标文件
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
1. **Codex 审查**（`run_in_background: true`）：
   - ROLE_FILE: `C:/Users/saite/.claude/.ccg/prompts/codex/reviewer.md`
   - 输入：变更的 Diff + 目标文件
   - 关注：安全性、性能、错误处理、逻辑正确性

2. **Gemini 审查**（`run_in_background: true`）：
   - ROLE_FILE: `C:/Users/saite/.claude/.ccg/prompts/gemini/reviewer.md`
   - 输入：变更的 Diff + 目标文件
   - 关注：可访问性、设计一致性、用户体验
