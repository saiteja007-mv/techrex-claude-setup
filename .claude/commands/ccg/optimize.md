...
| 模型 | 提示词 |
|------|--------|
| Codex | `C:/Users/saite/.claude/.ccg/prompts/codex/optimizer.md` |
| Gemini | `C:/Users/saite/.claude/.ccg/prompts/gemini/optimizer.md` |
...
1. 调用 `mcp__fast-context__fast_context_search` 检索目标代码（如可用）
2. 识别性能关键路径
3. 收集现有指标（如有）
...
1. **Codex 后端分析**：`Bash({ command: "...--backend codex...", run_in_background: true })`
   - ROLE_FILE: `C:/Users/saite/.claude/.ccg/prompts/codex/optimizer.md`
   - 需求：分析后端性能问题（$ARGUMENTS）
   - OUTPUT：性能瓶颈列表、优化方案、预期收益

2. **Gemini 前端分析**：`Bash({ command: "...--backend gemini...", run_in_background: true })`
   - ROLE_FILE: `C:/Users/saite/.claude/.ccg/prompts/gemini/optimizer.md`
